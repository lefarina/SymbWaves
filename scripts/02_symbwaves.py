...
Symbolic regression pipeline for ocean wave height prediction.
This version includes the hybrid regime model, the static "Star Formula" 
from the paper, and fixed visualization routines for the Lobster machine.
"""
import os
import gc
import sys
...
def predict_with_models(models: dict, X_test: np.ndarray, test_set: pd.DataFrame, cfg) -> np.ndarray:
    ...
    if getattr(cfg, 'use_static_formulas', False):
        print(f"  [INFO] Using STATIC Star Formula from the paper (Equation 8).")
...
def evaluate_performance(y_true, y_pred, test_set, cfg):
    ...
    print(f"MAPE(y) GERAL: {mape_geral:.2f}% | MAPE(Hs) GERAL: {mape_hs:.2f}%")
...
def generate_mean_maps(df, title_prefix, output_path, mape_value):
    ...
    plot_single_map(axes[2], lons, lats, grid_mape, 'MAPE (%) -- Avg: ' + f'{mape_value:.2f}%', 0, 100, cmap='Reds')
...
def generate_visualizations(models, results_df, metrics, cfg):
    ...
    plt.title("Model Performance Over Time"); plt.savefig(os.path.join(results_dir, 'performance_timeseries.png'), dpi=150); plt.close(fig)
    ...
    plt.figure(figsize=(12, 5)); mape_by_wa.plot(marker='o'); plt.title("MAPE vs. Wave Age"); plt.grid(True)
    plt.savefig(os.path.join(results_dir, 'mape_vs_wave_age.png'), dpi=150); plt.close()
    
    # Basin Maps
    generate_mean_maps(results_df, "Overall", os.path.join(results_dir, 'mean_map_overall.png'), metrics['mape_geral'])
    generate_mean_maps(results_df[results_df['Wave_age'] <= wa_y], "Wind-Sea", os.path.join(results_dir, 'mean_map_windsea.png'), metrics['mape_geral'])
    generate_mean_maps(results_df[results_df['Wave_age'] >= wa_o], "Swell", os.path.join(results_dir, 'mean_map_swell.png'), metrics['mape_geral'])
...
def main():
    ...
    if config.new_train:
        ...
        print(f"  Loading models/settings for prediction mode...")
    ...
    # --- SUMMARY BLOCK (RESTORED & IMPROVED) ---
    print("\n--- Model Configuration Summary ---")
    wa_y_eff = thresholds['wa_y']
    wa_o_eff = thresholds['wa_o']
    if getattr(config, 'use_static_formulas', False):
        print("  MODE: STATIC (Hardcoded Star Formulas)")
        print(f"  YOUNG Model (WA <= {wa_y_eff:.2f}): y = 0.20444 * Wave_age")
        print(f"  STABLE SWELL Model ({wa_o_eff:.2f} <= WA < {config.swell_stability_threshold}): y = (0.242 * WA)^sqrt(2.686 - mdts_cos)")
    else:
        print("  MODE: DYNAMIC (PySR Models)")
        if 'young' in models: print(f"  YOUNG Model discovered: {models['young'].latex()}")
        if 'swell' in models: print(f"  STABLE SWELL Model discovered: {models['swell'].latex()}")
